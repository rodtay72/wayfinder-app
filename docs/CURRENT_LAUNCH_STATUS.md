...
**Next owner action:** Post-merge **production smoke** on `wayfinder-modular.vercel.app` after Vercel deploy — see [MHP_OWNER_HANDOFF_RUNBOOK.md](./MHP_OWNER_HANDOFF_RUNBOOK.md) § PR #138 post-merge production smoke.

**PR #138 (merged):** Email-bound MHP invite acceptance is on **main**. **Before merge**, production `/counsellor.html` showing access-denied or missing email-bound consume behaviour was **expected** — main did not yet contain PR #138 runtime. **Do not** add token/`sessionStorage` patches.

**Owner prerequisites (confirm before smoke):**

1. [supabase-mhp-invite-token-acceptance-contract.sql](../supabase-mhp-invite-token-acceptance-contract.sql) (PR #132) applied in Supabase SQL Editor.
2. [supabase-mhp-invite-email-bound-acceptance-contract.sql](../supabase-mhp-invite-email-bound-acceptance-contract.sql) (PR #138) applied **after** PR #132 contract.

**Current owner blocker:** Post-merge production smoke from clean state — invitee `rodney@thegreenhouse.sg`, owner/admin `rodney@psytec.com.sg` — full checklist in runbook. Supabase Auth email delivery (Custom SMTP, redirect allow list for `counsellor.html**` including `?mhp_setup=profile`) still required for confirmation email. **Payment gateway remains paused** until post-merge production smoke passes.

**Non-negotiables:** No browser `profiles.insert`/`profiles.upsert`. No service-role keys in browser. Do not weaken Supabase auth, RLS, email verification, `ensure_profile`, Parent/Child IDs, journal save/read, privacy masking, MHP publication/licence rules, or active-membership journal-read gate.
...
| PR #138 Email-bound MHP invite acceptance | Token opens invite page only; verified email controls post-signup acceptance; email-bound consume RPC | Complete (merged) — post-deploy production smoke pending |
...
- **MHP invite email-bound acceptance (PR #138)** — **merged on main** — owner must confirm PR #132 + PR #138 SQL applied; **post-merge production smoke pending** from clean state (`rodney@thegreenhouse.sg` invitee, `rodney@psytec.com.sg` owner/admin) — see runbook § PR #138 post-merge production smoke. Pre-merge production access-denied was expected. **Payment gateway remains paused.**
...
- **Payment / entitlement runtime** — **not started** — strategy spec merged in [PAYMENT_GATEWAY_AND_PRICING_STRATEGY.md](./PAYMENT_GATEWAY_AND_PRICING_STRATEGY.md) (PR #120A); **remains paused until post-merge MHP invite production smoke passes**
...
- **MHP invite request pipeline (PR #128–#138)** — **PR #129–#138 merged** — email-bound acceptance on production after Vercel deploy + owner SQL apply
...
16. MHP invite acceptance (after PR #138 merge + SQL apply + Vercel deploy): full post-merge checklist in [MHP_OWNER_HANDOFF_RUNBOOK.md](./MHP_OWNER_HANDOFF_RUNBOOK.md) § PR #138 post-merge production smoke — invitee `rodney@thegreenhouse.sg`, owner/admin `rodney@psytec.com.sg`, clean state, newest production invite link, email-bound consume, draft/hidden profile, `pending_review` membership, journal read gate.
